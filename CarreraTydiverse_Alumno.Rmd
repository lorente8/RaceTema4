...
```{r}
#transformamos los datos en un tibble
cran <- as_tibble(mydf)
#borramos el data frame original
rm(mydf)
```
...
```{r}
#Observamos la visualización
cran
#usamos la funcion glimpse sobre cran
glimpse(cran)
```
# dplyr
...
```{r}
#cargamos lalibreria dplyr
library(dplyr)
#elegimos las variables `ip_id, package, country`
cran %>%
...
```{r}
#seleccionamos todas las variables entre r_arch y country
cran %>%
  select(r_arch:country)
```
5.2 Puedes usar el signo (-) para no coger una columna determinada. Selecciona todas las columnas menos las que están entre `date y size` 
```{r}
#Seleccionamos todas las columnas menos las que están entre date y size
cran %>%
  select(-date:-size)
...
```{r}
cran %>%
  filter(package == "plotly" & r_arch == "3.5.1")


```
6.2 Elige aquellas descargas realizadas desde España (ES), con versiones de R superiores a la 3.4.0
```{r}
cran %>%
  filter( country == "ES" & r_arch > "3.4.0")
```
...
```{r}
#elegir la versión del sistema operativo no está vacía
cran %>%
  filter(nchar(r_os) > 0)
```
10. Almacena en `cran2` todas las columnas entre `size` y `ip_id`. 
```{r}
#guardamos en la variable cran
cran2 <- cran %>%
    select( size:ip_id)
```
...
cran2 %>%
  arrange(desc(ip_id))
```
12.2 Ordena `cran2` según los valores de `package e ip_id`.
```{r}
cran2 %>%
